...
function get_single_join_post($id) {
    global $db;
    $q = $db->prepare("SELECT p.id, title, content, p.image post_image, p.created_at, u.name, u.firstname, u.role, ua.other, ua.image user_image FROM post p LEFT JOIN user u ON p.user_id = u.id LEFT JOIN user_add ua on u.id = ua.user_id WHERE p.id = :id");
    $q->execute(['id' => $id]);
    return $q->fetch(PDO::FETCH_OBJ);
}
